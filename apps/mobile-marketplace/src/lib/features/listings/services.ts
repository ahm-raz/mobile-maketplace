...
/** Lowest `position` image per listing (for cards and home grids). */
export async function mapListingPrimaryImageUrls(listingIds: string[]): Promise<Map<string, string>> {
	...
	for (const row of data as Array<{ listing_id: string; url: string; position: number }>) {
		const cur = best.get(row.listing_id);
		if (!cur || row.position < cur.position) {
			best.set(row.listing_id, { url: row.url, position: row.position });
		}
	}
